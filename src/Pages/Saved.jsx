...
function Saved() {
  const {currency,sort, favoriteCoins, addCoinToFavorites, removeCoinFromFavorites, isCoinInFavorites} = useContext(cryptoContext)
  const [isOpen, setIsOpen] = useState(false);
  const {setCoinId,  setSelectedCryptoId,coinData} = useContext(cryptoContext)
...
return (
    <div className='flex flex-col  mb-8 items-center  justify-center   mt-7  '>
            <table className='w-full table-auto  px-3 rounded-md'>   
   {
    favoriteCoins.length !== 0 ? (
      ...
      <tr >
              <th className='py-4 px-[2rem]'>Asset</th>
              <th className='py-4 px-[2rem] md:table-cell hidden'>Name</th>
              <th className='py-4 px-[2rem]'>Price</th>
              <th className='py-4 px-[2rem] md:table-cell hidden'>Total_volume</th>
              <th className='py-4 px-[2rem]'>{sort}</th>
              <th className='py-4 px-[2rem] lg:table-cell hidden'>1H</th>
              <th className='py-4 px-[2rem] lg:table-cell hidden'>24Hrs</th>
              <th className='py-4 px-[2rem] lg:table-cell hidden'>7Days</th>
          </tr>
          </thead>
    ) :(
      null
            ...
            <tbody  className={`${favoriteCoins ? ' rounded-md bg-slate-900 border-1 z-50 border-slate-900 shadow-md ' : 'bg-none'}`}>
          {
          favoriteCoins.length !== 0 ? 
          favoriteCoins.map((item, index) => (
                <tr key={index} className=' hover:bg-slate-700 pl-3 text-center text-base border-0   hover:ease-in-out'>
                  <div  className='w-full h-full'>
                    <td   onClick={()=>{openModal(item.id)}} className='py-4 cursor-pointer flex uppercase pl-2  md:text-sm text-xs  gap-[0.4rem] md:gap-[0.7rem]'>
            ...
            : 
            <span>
                <span className='flex   h-full items-center justify-center   mt-[3rem] '>
               <span className='text-slate-100 text-2xl font-rajdhani'>
                    No coin Available in your favorites yet
                </span>
        ...
        check out our coins <Link to='/' className='text-green-500 inline-block ml-3 cursor-pointer'>here</Link>  
        </div>
                </span>
            }
        </tbody>
